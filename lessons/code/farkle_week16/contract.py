"""Explicit CS2 strategy contract layered over CS1's function contract."""
...
class Strategy(ABC):
    ...
    def decide(self, state):
        """Return ``roll`` or ``bank`` for one decision state."""
...
class FunctionStrategyAdapter(Strategy):
    """Adapt the inherited CS1 function strategy without rewriting it."""
...
class ValidatingStrategy(Strategy):
    """Composition wrapper that enforces the contract around any callable."""

    def __init__(self, wrapped, name=None):
        self.wrapped = wrapped
        self.name = name or getattr(wrapped, "name", getattr(wrapped, "__name__", "strategy"))
        self.complexity_note = getattr(wrapped, "complexity_note", "wrapped callable")

    def decide(self, state):
        action = self.wrapped(state)
        if action not in VALID_ACTIONS:
            raise ValueError(
                f"{self.name} returned {action!r}; expected 'roll' or 'bank'"
            )
