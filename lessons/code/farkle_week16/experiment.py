...
def build_strategy(specification, seed):
    """Build one fixed strategy family and measure preparation cost."""
    ...
    if specification.startswith("learner:"):
        ...
        table = learner.ExperienceTable(epsilon=0.2, seed=seed)
        learner.train(table, num_turns=turns, seed=seed)
...
def run_experiment(config):
    ...
    summary = simulate.run_many_games(
        built_a.strategy, built_b.strategy, config.games, config.seed
    )
...
def default_suite(games=500, seed=1):
    ...
    return [
        ...
        ExperimentConfig("learner:20000", games=games, seed=seed + 20),
        ExperimentConfig("rollout:25", games=games, seed=seed + 30),
        ExperimentConfig("rollout:100", games=games, seed=seed + 40),
    ]
